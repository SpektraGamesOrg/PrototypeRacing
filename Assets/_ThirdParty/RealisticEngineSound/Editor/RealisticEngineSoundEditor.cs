...
namespace SkrilStudio
{
    ...
    public class RealisticEngineSoundEditor : Editor
    {
        ...
        private const string res2EditorFolderGUID = "a51ca3bf93e83f94381417806f99c1e9"; // .\RealisticEngineSound\Editor folder ID
        private const string res2AssetsFolderGUID = "ee29c351e83fc444787d7766fc6b1d03"; // .\RealisticEngineSound\Assets folder ID
        private void OnEnable()
        {
            string assetPathEditor = AssetDatabase.GUIDToAssetPath(res2EditorFolderGUID);
            texLogo = (Texture2D)AssetDatabase.LoadAssetAtPath(assetPathEditor + "/logo.png", typeof(Texture2D));
            // Find the "Hidden/Internal-Colored" shader, and cache it for use.
            material = new Material(Shader.Find("Hidden/Internal-Colored"));

            // welcome message
            if (EditorPrefs.HasKey("RES2Welcome"))
                enableNotify = EditorPrefs.GetBool("RES2Welcome");
            else
                enableNotify = true; // brand new user
        ...
        public override void OnInspectorGUI()
        {
            ...
            string assetPath = AssetDatabase.GUIDToAssetPath(res2AssetsFolderGUID);
            ...
            if (editClips)
            {
                ...
                if (GUILayout.Button(new GUIContent("Load Interior Audio Clips From ../" + res.prefabName + "/Interior or Helmet Folder", "It will load those audio files of the engine sound pack that are made for interior camera view.")))
                {
                    //res.enabled = false;
                    isInterior = true;
                    res.isInterior = true;
            ...
            if (editSettings)
            {
                ...
                GUI.backgroundColor = new Color(1f, 1f, 1f);
                res.masterVolume = EditorGUILayout.Slider(new GUIContent("Master Volume", "Sets the maximum volume an audio source can reach."), res.masterVolume, 0, 2);
                ...
                if (res.enableReverseGear)
                {
                    ...
                    if (!res.customReverseClip) // preset reverse clip
                    {
                        ...
                        if (EditorGUIUtility.Load(assetPath + "/Sounds/Reversing/reversing_" + res.reverseClipID + ".wav") == null || EditorGUIUtility.Load(assetPath + "/Sounds/Reversing/Interior/int_reversing_" + res.reverseClipID + ".wav") == null)
                        {
                ...
                if (res.enableReverseBeep)
                {
                    ...
                    if (!res.customReverseBeepClip) // preset reverse clip
                    {
                        ...
                        // show file missing error
                        if (EditorGUIUtility.Load(assetPath + "/Sounds/Reversing/beep_" + res.reverseBeepClipID + ".wav") == null || EditorGUIUtility.Load(assetPath + "/Sounds/Reversing/Interior/int_beep_" + res.reverseBeepClipID + ".wav") == null)
                        {
                            redTextSizeGuiStyle.normal.textColor = Color.red;
                            redTextSizeGuiStyle.alignment = TextAnchor.LowerLeft;
                            GUILayout.Label("file missing!", redTextSizeGuiStyle, GUILayout.Width(55), GUILayout.Height(18));
                ...
                if (res.windNoiseEnabled == DynamicSoundController.WindNoiseEnum.On)
                {
                    ...
                    if (!res.customWindClip) // preset wind clip
                    {
                        ...
                        // show file missing error
                        if (EditorGUIUtility.Load(assetPath + "/Sounds/Wind_Sounds/wind_" + res.windClipID + ".wav") == null || EditorGUIUtility.Load(assetPath + "/Sounds/Wind_Sounds/Interior/int_wind_" + res.windClipID + ".wav") == null)
                        {
                            redTextSizeGuiStyle.normal.textColor = Color.red;
                            redTextSizeGuiStyle.alignment = TextAnchor.LowerLeft;
                            GUILayout.Label("file missing!", redTextSizeGuiStyle, GUILayout.Width(55), GUILayout.Height(18));
                ...
                enableNotify = EditorGUILayout.Toggle(new GUIContent("Show Welcome Information", "Will show the welcome information in the inspector. This can be disabled, and it is saved globally for all Unity projects using this asset."), enableNotify);
                EditorGUILayout.EndVertical();
            }
            GUI.backgroundColor = new Color(.7f, .7f, .7f);
            if (res.curvesByVolume == RealisticEngineSound.CurvesByVolume.ShowRawCurves)
        ...
        {
            // save
            EditorPrefs.SetBool("RES2Welcome", enableNotify);
            _enableNotify = enableNotify;
